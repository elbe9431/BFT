...
class BFT_Mail
{
    ...
    const bank_msg          = "Um Test 2 - Berufswahl der Hamburger paartherapietests nutzen zu können haben Sie die Bezahloption Banküberweisung gewählt.\n\nBitte überweisen Sie einmalig den Betrag von %s Euro unter Angabe Ihrer E-Mail-Adresse und Ihres Nachnamens auf folgendes Konto:\n\nKontoinhaber: Enno Heyken\nKreditinstitut: Netbank\nBankleitzahl: 20090500\nKontonummer: 7380194\n\nWir senden Ihnen dann eine Mail, sobald wir den Zahlungseingang verbucht haben.\n\nNachfolgend senden wir Ihnen noch die aktuellen Teilnahmebedingungen des Tests.";
    
    // Ergebnis
    const ergebnis_subject  = "Ergebnis Test 2 Berufswahl";
    const ergebnis_msg      = "Sie haben Test 2 Berufswahl abgeschlossen und damit die beiden Hamburger paartherapietests unter http://www.paartherapietest.eu komplett durchlaufen. Im Anhang erhalten Sie das Ergebnis mit weiterführenden Informationen.";
    
    // Ergebnis
    const ergebnis1_subject = "Ergebnis Test 1: Berufsinteressen und Kennwort Test 2: Berufswahl";
    ...
    private function Send($userid)
    {
        ...
        if ($this->attachments) {
          $this->msg .= "\n\n--" . $this->boundary . "\n";
          $this->msg .= "Content-Type: application/octetstream; name=\"".$this->file_name."\"\n";
          $this->msg .= "Content-Transfer-Encoding: base64\n";
          $this->msg .= "Content-Disposition: attachment; filename=\"".$this->file_name."\"\n\n";
          $this->msg .= $this->file_content;
        }
        $this->msg .= "\n\n--".$this->boundary."--";
        
        if(!mail($row['email'], $this->subject, $this->msg, $this->headers))
        {
          error_log("[DEBUG] ".print_r($this,true));
          error_log("[DEBUG] TO: ".print_r($row['email'],true));
        }
    }
    ...
    public function SendBankueberweisungNachricht($userid)
    {
        ...
        if(!mail($this->to, $this->subject, wordwrap($this->msg), $this->headers))
        {
          error_log("[DEBUG] ".print_r($this,true));
        }
    }
